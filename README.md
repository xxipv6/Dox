...
- **端口转发建议 ✅**：终端输出出现 `localhost:端口` 服务横幅时，右下角弹「转发到本机」气泡，一键建成 ssh -L（字节级门控，常规输出零开销）；**容器感知**：远端容器标签里的建议会用 `docker inspect` 解析网桥 IP 当转发目标，没发布端口的容器也能转；本地终端/本机容器不弹（localhost 本来就能开）。设置里可关
