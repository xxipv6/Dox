...
func TestParseProcStat(t *testing.T) {
	...
	pid, ppid, utime, stime, rssPages, comm, ok := parseProcStat(line)
...
func TestParseProcStatCommWithSpacesAndParens(t *testing.T) {
	...
	pid, ppid, utime, stime, _, comm, ok := parseProcStat(line)
...
func TestParseProcStatGarbage(t *testing.T) {
	if _, _, _, _, _, _, ok := parseProcStat("not a stat line"); ok {
		t.Fatal("垃圾输入必须判失败")
	}
	if _, _, _, _, _, _, ok := parseProcStat(""); ok {
		t.Fatal("空输入必须判失败")
	}
}
