...
check('右键弹出了菜单', items.length === 3, JSON.stringify(items))
check('菜单是「下载 / 重命名 / 删除」', items.map((i) => i.text).join('/') === '下载/重命名/删除', items.map((i) => i.text).join('/'))
...
check('多选时重命名不可点', items[1]?.disabled === true, JSON.stringify(items[1]))
check('删除也带上数量', items[2]?.text === '删除这 2 项', String(items[2]?.text))
